...
export default function Viewmaterial() {
  ...
  let deleterow = () => {
    if (allids.length >= 1) {
      swal({
        title: "Are you sure?",
        text: "Once deleted, you will not be able to recover these materials!",
        icon: "warning",
        buttons: true,
        dangerMode: true,
      })
          ...
          axios
            ...
            .then((fainlrec) => {
              if (fainlrec.status) {
                getMaterial();
                setallids([]);
                swal("Poof! Your materials have been deleted!", {
                  icon: "success",
                });
              }
            });
  ...
  let changestatus = () => {
    if (allids.length >= 1) {
      axios
        .post(`${apibaseurl}/material/change-status`, { ids: allids })
        .then((rec) => rec.data)
        .then((finalrec) => {
          if (finalrec.status) {
            getMaterial(); // Fixed function name from getcolor to getMaterial
            setallids([]);
  ...
  let activdata = date.filter((v) => v.materialstatus).length; // Fixed status key
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-2">Manage product materials and types.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-gray-500 text-sm font-medium">Total Materials</p>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className={`relative transition-all duration-300 ${searchbox ? 'w-full sm:w-72 opacity-100' : 'w-0 opacity-0 overflow-hidden'}`}>
            <RiSearchLine onClick={getMaterial} className="absolute left-3 top-3.5 text-gray-400" />
            <input
              type="search"
              onChange={(e)=>{
                let obj={...searchObj}
              ...
              {date.length >= 1 ? (
                date.map((obj, index) => (
                  <tr key={index} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-6 py-4 font-medium text-gray-900">{obj.materialName}</td>
                    <td className="px-6 py-4 text-gray-600">{obj.materialOder}</td>
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 text-xs font-semibold rounded-full ${obj.materialstatus ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {obj.materialstatus ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <Link to={`/Material_update/${obj._id}`}>
