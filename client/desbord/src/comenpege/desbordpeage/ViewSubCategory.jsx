...
export default function Viewsubcategory() {
  ...
  let getsubcategory = () => {
    axios
      .get(`${apibaseurl}/Subcategory/viwe`,{
          params:searchObj
        })
      .then((rec) => rec.data)
      .then((finlerec) => {
  ...
  let deleterow = () => {
    if (allids.length >= 1) {
      swal({
        title: "Are you sure?",
        text: "Once deleted, you will not be able to recover these subcategories!",
        icon: "warning",
        buttons: true,
        dangerMode: true,
      })
          ...
          axios
            ...
            .then((fainlrec) => {
              if (fainlrec.status) {
                getsubcategory();
                setallids([]);
                swal("Poof! Your subcategories have been deleted!", {
                  icon: "success",
                });
              }
            });
  ...
  let changestatus = () => {
    if (allids.length >= 1) {
      axios
        .post(`${apibaseurl}/Subcategory/change-status`, { ids: allids })
        .then((rec) => rec.data)
        .then((finalrec) => {
          if (finalrec.status) {
            getsubcategory();
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-2">Organize product sub-categories efficiently.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-gray-500 text-sm font-medium">Total Sub Categories</p>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className={`relative transition-all duration-300 ${searchbox ? 'w-full sm:w-72 opacity-100' : 'w-0 opacity-0 overflow-hidden'}`}>
            <RiSearchLine onClick={getsubcategory} className="absolute left-3 top-3.5 text-gray-400" />
            <input
              type="search"
              onChange={(e)=>{
                let obj={...searchObj}
              ...
              {date.length >= 1 ? (
                date.map((obj, index) => (
                  <tr key={index} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-gray-600 font-medium">{obj.Category?.categoryName}</td>
                    <td className="px-6 py-4 font-semibold text-gray-900">{obj.SubcategoryName}</td>
                    <td className="px-6 py-4">
                      <img 
                        src={path + obj.Subcategoryimg} 
                        alt={obj.SubcategoryName} 
                        className="h-10 w-10 rounded-lg object-cover border border-gray-200 shadow-sm"
                      />
                    </td>
                    <td className="px-6 py-4 text-gray-600">{obj.SubcategoryOder}</td>
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 text-xs font-semibold rounded-full ${obj.Subcategorystatus ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {obj.Subcategorystatus ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <Link to={`/Update-SubCategory/${obj._id}`}>
                ...
                <tr>
                  <td colSpan="7" className="px-6 py-10 text-center text-gray-500">
                    No subcategories found. Add a new one to get started.
                  </td>
                </tr>
              )}
            </tbody>
