...
export default function Viewcategory() {
  ...
  let changestatus = () => {
    if (allids.length >= 1) {
      axios
        .post(`${apibaseurl}/category/change-status`, { ids: allids })
        .then((rec) => rec.data)
        .then((finalrec) => {
          if (finalrec.status) {
            getcategory();
  ...
  let activdata = date.filter((v) => v.categorystatus).length;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-2">Organize and manage your product categories efficiently.</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className={`relative transition-all duration-300 ${searchbox ? 'w-full sm:w-72 opacity-100' : 'w-0 opacity-0 overflow-hidden'}`}>
            <RiSearchLine onClick={getcategory} className="absolute left-3 top-3.5 text-gray-400" />
            <input
              type="search"
              name="categoryName"
               onChange={(e)=>{
              ...
              {date.length >= 1 ? (
                date.map((obj, index) => (
                  <tr key={index} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-gray-600">{obj.categoryOder}</td>
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 text-xs font-semibold rounded-full ${obj.categorystatus ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {obj.categorystatus ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <Link to={`/catupdete/${obj._id}`}>
                ...
                <tr>
                  <td colSpan="6" className="px-6 py-10 text-center text-gray-500">
                    No data found. Add a new category to get started.
                  </td>
                </tr>
              )}
            </tbody>
