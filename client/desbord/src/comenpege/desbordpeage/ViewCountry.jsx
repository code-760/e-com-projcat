...
export default function Viewcountry() {
  ...
  let deleterow = () => {
    if (allids.length >= 1) {
      swal({
        title: "Are you sure?",
        text: "Once deleted, you will not be able to recover these countries!",
        icon: "warning",
        buttons: true,
        dangerMode: true,
      })
          ...
          axios
            ...
            .then((fainlrec) => {
              if (fainlrec.status) {
                getcountry();
                setallids([]);
                swal("Poof! Your countries have been deleted!", {
                  icon: "success",
                });
              }
            });
  ...
  let changestatus = () => {
    if (allids.length >= 1) {
      axios
        .post(`${apibaseurl}/Country/change-status`, { ids: allids })
        .then((rec) => rec.data)
        .then((finalrec) => {
          if (finalrec.status) {
            getcountry(); // assuming getcolor was a typo in original code
            setallids([]);
  ...
  let activdata = date.filter((v) => v.Countrystatus).length;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-2">Manage supported countries and locations.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
          <div>
            <p className="text-gray-500 text-sm font-medium">Total Countries</p>
              ...
              {date.length >= 1 ? (
                date.map((obj, index) => (
                  <tr key={index} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-6 py-4 font-medium text-gray-900">{obj.CountryName}</td>
                    <td className="px-6 py-4 text-gray-600">{obj.CountryOder}</td>
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 text-xs font-semibold rounded-full ${obj.Countrystatus ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {obj.Countrystatus ? 'Active' : 'Inactive'}
                      </span>
                    </td>
