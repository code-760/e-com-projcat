...
export default function Viewuser() {
  ...
  let deleterow = () => {
    if (allids.length >= 1) {
      swal({
        ...
        if (willDelete) {
          axios
            .post(`${apibaseurl}/user/user-multidelete`, { ids: allids })
            .then((rec) => rec.data)
            .then((fainlrec) => {
              if (fainlrec.status) {
                viweuser();
  ...
  let changestatus = () => {
    if (allids.length >= 1) {
      axios
        .post(`${apibaseurl}/user/user-changestatus`, { ids: allids })
        .then((rec) => rec.data)
        .then((finalrec) => {
          if (finalrec.status) {
            viweuser();
            setallids([]);
            toast.success("Status updata successfully");
          }
        });
    } else {
      toast.error("Please check the box");
    ...
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          {/* Search Box */}
          <div
            className={`relative transition-all duration-300 ${searchbox ? "w-full sm:w-72 opacity-100" : "w-0 opacity-0 overflow-hidden"}`}
          >
            <RiSearchLine onClick={viweuser} className="absolute left-3 top-3.5 text-gray-400" />
            <input
              type="search"
              name="UserName"
               onChange={(e)=>{
              ...
              <tr className="bg-gray-50 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                <th className="px-6 py-4">
                  <input
                    ...
                    checked={data?.length > 0 && data?.length === allids.length}
                    type="checkbox"
                    className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                  />
                </th>
                <th className="px-6 py-4">UserName</th>
                <th className="px-6 py-4">useremail</th>
              ...
              {data?.length > 0 ? (
                data.map((obj, index) => (
                  <tr className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-gray-600">{obj.useremail}</td>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-3 py-1 text-xs font-semibold rounded-full ${obj.Userstatus ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                      >
                        {obj.Userstatus ? "Active" : "Inactive"}
                      </span>
                    </td>
                  </tr>
                ))
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50 flex justify-between items-center">
          <p className="text-sm text-gray-500">{data?.length} entries</p>
        </div>
      </div>
    </div>
  );
