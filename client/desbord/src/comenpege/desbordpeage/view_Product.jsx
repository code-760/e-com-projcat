    ...
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      ...
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-gray-800 flex items-center gap-3">
          Product Management
          <span className="text-sm font-normal text-gray-500 bg-gray-200 px-2 py-1 rounded-full">{date.length} Products</span>
        </h2>
        <p className="text-gray-500 mt-2">View, edit, and manage your product inventory.</p>
      </div>
            ...
            <thead>
              <tr className="bg-gray-50 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                <th className="px-6 py-4">
                  <input type="checkbox" onChange={getallchecked} className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                </th>
            ...
            <tbody className="divide-y divide-gray-100">
              {date.map((obj, index) => (
                <tr key={index} className="hover:bg-gray-50 transition-colors group">
                  ...
                  <td className="px-6 py-4 font-medium text-gray-900">{obj.ProductName}</td>
                  ...
                  <td className="px-6 py-4 text-gray-500 max-w-xs truncate">
                    {parse(obj.Description ? obj.Description.substring(0, 50) + "..." : "")}
                    <button onClick={() => gatdiles(obj._id)} className="text-indigo-500 hover:underline ml-1 text-xs font-medium">Read more</button>
                  </td>
                  <td className="px-6 py-4">
                    <img src={path + obj.ProductImage} alt="Product" className="h-12 w-12 rounded-lg object-cover border border-gray-200 shadow-sm" />
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 text-xs font-semibold rounded-full ${obj.Productstatus ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                      {obj.Productstatus ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => gatdiles(obj._id)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="View Details">
                        <RiEyeLine />
                      </button>
                      ...
                      <button onClick={deleterow} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
                        <RiDeleteBin6Line />
                      </button>
                    </div>
                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="flex justify-between items-center p-6 border-b border-gray-100">
              ...
              <button onClick={() => setread(false)} className="text-gray-400 hover:text-gray-600 transition-colors">
                <MdClose size={24} />
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="aspect-square w-full border border-gray-200 rounded-xl overflow-hidden bg-gray-50">
                    <img src={path + detail.ProductImage} alt="Main" className="w-full h-full object-contain mix-blend-multiply" />
                  </div>
                  
                  {/* Gallery */}
                  <div className="grid grid-cols-4 gap-2">
                    {detail.GalleryImage?.map((img, idx) => (
                      <div key={idx} className="aspect-square border border-gray-200 rounded-lg overflow-hidden bg-gray-50 cursor-pointer hover:border-indigo-500 transition-all">
                        <img src={path + img} alt={`Gallery ${idx}`} className="w-full h-full object-cover" />
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900">{detail.ProductName}</h2>
                    <p className="text-sm text-gray-500 mt-1">ID: {detail._id}</p>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 p-4 rounded-xl">
                      <p className="text-xs text-gray-500 font-bold uppercase">Price</p>
                      <p className="text-xl font-bold text-indigo-600">₹{detail.SalePrice}</p>
                      {detail.ActualPrice && <p className="text-sm text-gray-400 line-through">₹{detail.ActualPrice}</p>}
                    </div>
                    <div className="bg-gray-50 p-4 rounded-xl">
                      <p className="text-xs text-gray-500 font-bold uppercase">Stock</p>
                      <p className="text-xl font-bold text-gray-800">{detail.TotalInStocks || "N/A"}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between py-2 border-b border-gray-100">
                      ...
                      <span className="text-gray-900">{detail.Category?.categoryName}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-gray-100">
                      <span className="text-gray-600 font-medium">Sub-Category</span>
                      <span className="text-gray-900">{detail.SubCategory?.SubcategoryName}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-gray-100">
                      <span className="text-gray-600 font-medium">Colors</span>
                      <div className="flex gap-2">
                        {detail.color?.map((c, i) => (
                          <span key={i} className="px-2 py-1 bg-gray-100 text-xs rounded-md text-gray-700 border border-gray-200">
                            {c.colorName}
                          </span>
                        ))}
                      </div>
            ...
            <div className="p-4 border-t border-gray-100 bg-gray-50 flex justify-end">
              <button onClick={() => setread(false)} className="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 font-medium transition-colors">
                Close
              </button>
            </div>
