...
export default function View_testimonial() {
  ...
  let gettestimonial = () => {
    axios
      .get(`${apibaseurl}/Tastimonial/viwe`,{
          params:searchObj
        })
      .then((rec) => rec.data)
      .then((finlerec) => {
  ...
  let deleterow = () => {
    if (allids.length >= 1) {
      swal({
        title: "Are you sure?",
        text: "Once deleted, you will not be able to recover these testimonials!",
        icon: "warning",
        buttons: true,
        dangerMode: true,
      })
      .then((willDelete) => {
        if (willDelete) {
          axios
            .post(`${apibaseurl}/Tastimonial/multidelete`, { ids: allids })
            .then((rec) => rec.data)
            .then((fainlrec) => {
              if (fainlrec.status) {
                gettestimonial();
                setallids([]);
                swal("Poof! Your testimonials have been deleted!", {
                  icon: "success",
                });
              }
            });
  ...
  let changestatus = () => {
    if (allids.length >= 1) {
      axios
        .post(`${apibaseurl}/Tastimonial/change-status`, { ids: allids })
        .then((rec) => rec.data)
        .then((finalrec) => {
          if (finalrec.status) {
            gettestimonial();
  ...
  let activdata = date.filter((v) => v.Tastimonialstatus).length;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-2">Manage customer reviews and feedback.</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className={`relative transition-all duration-300 ${searchbox ? 'w-full sm:w-72 opacity-100' : 'w-0 opacity-0 overflow-hidden'}`}>
            <RiSearchLine onClick={gettestimonial} className="absolute left-3 top-3.5 text-gray-400" />
            <input
              type="search"
              onChange={(e)=>{
                let obj={...searchObj}
              ...
              {date.length >= 1 ? (
                date.map((obj, index) => (
                  <tr key={index} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-gray-600">{obj.Order}</td>
                    <td className="px-6 py-4 text-gray-600 max-w-xs truncate" title={obj.Description}>{obj.Description}</td>
                    <td className="px-6 py-4 text-yellow-500 font-bold">{obj.Rating} ★</td>
                    <td className="px-6 py-4 text-gray-500 italic max-w-xs truncate" title={obj.Message}>"{obj.Message}"</td>
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 text-xs font-semibold rounded-full ${obj.Tastimonialstatus ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {obj.Tastimonialstatus ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <Link to={`/Update_Testimonial/${obj._id}`}>
