...
export default function ProductListing() {
  ...
  const [sidebarData, setSidebarData] = useState({ categories: [], subcategories: [], materials: [], colors: [] });
  ...
  }, [selectedCats, selectedMats, selectedColors, price, sort, Baseurl]);
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="max-w-[1440px] mx-auto grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-4 lg:p-6">
        ...
        `}>
          ...
          <h1 className="hidden lg:block font-bold text-2xl mb-6 border-b pb-2">Filters</h1>

          {/* Price Range */}
          <div className="mb-8">
            <h2 className="font-bold text-sm uppercase tracking-widest text-gray-400 mb-4">Price Range</h2>
            <input
              type="range" min="0" max="200000" step="5000" value={price}
            ...
            <div className="flex justify-between text-xs mt-3 font-bold">
              ...
              <span className="text-[#c09578] bg-[#c09578]/10 px-2 py-1 rounded">Up to: ₹{price}</span>
            </div>
          </div>

          {/* Categories */}
          <div className="mb-6">
            <h2 className="font-bold text-sm uppercase tracking-widest text-gray-400 mb-4">Categories</h2>
            {sidebarData.categories.map((cat) => (
              <div key={cat._id} className="mb-5">
                <h3 className="text-xs font-black text-gray-900 mb-3">{cat.categoryName}</h3>
                    ...
                    .map((sub) => (
                      <label key={sub._id} className="flex items-center group cursor-pointer">
                        <input
                          ...
                          className="w-4 h-4 rounded border-gray-300 accent-[#c09578] mr-3"
                        />
                        <span className="text-gray-600 text-sm group-hover:text-black transition-colors">{sub.SubcategoryName}</span>
                      </label>
                    ))}
                </div>
              </div>
        ...
        <main className="px-4 lg:px-0">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8">
              {products.map((item) => (
                <div key={item._id} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-gray-100 flex flex-col h-full">
                  ...
                  <div className="p-5 flex flex-col flex-grow text-center">
                    ...
                    <div className="mt-auto space-y-4">
                      <div className="flex justify-center items-center gap-3">
                        <span className="text-gray-400 line-through text-xs italic">₹{item.ActualPrice}</span>
                        <span className="text-lg font-black text-gray-900 font-[cha]">₹{item.SalePrice}</span>
                      </div>
          ...
          {!loading && products.length === 0 && (
            <div className="text-center py-32 bg-white rounded-3xl border-2 border-dashed border-gray-100">
              <p className="text-gray-400 font-bold text-xl italic">No luxury pieces match your selection.</p>
              <button onClick={() => {setSelectedCats([]); setSelectedMats([]); setPrice(200000);}} className="mt-4 text-[#c09578] font-bold underline">Clear All Filters</button>
            </div>
          )}
        </main>
      </div>
      ...
      {isMobileFilterOpen && <div onClick={() => setIsMobileFilterOpen(false)} className="fixed inset-0 bg-black/40 backdrop-blur-sm z-30 lg:hidden transition-opacity" />}
    </div>
  );
}